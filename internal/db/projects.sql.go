...
const searchProjectsCount = `-- name: SearchProjectsCount :one
SELECT COUNT(*) 
FROM project p
JOIN client c ON p.client_id = c.id
WHERE p.deleted_at IS NULL AND c.deleted_at IS NULL 
...
type SearchProjectsCountParams struct {
	Name   string `json:"name"`
	Name_2 string `json:"name_2"`
}

func (q *Queries) SearchProjectsCount(ctx context.Context, arg SearchProjectsCountParams) (int64, error) {
	row := q.db.QueryRowContext(ctx, searchProjectsCount, arg.Name, arg.Name_2)
	var count int64
	err := row.Scan(&count)
	return count, err
}

const searchProjectsWithClientPagination = `-- name: SearchProjectsWithClientPagination :many
SELECT p.id, p.name, p.client_id, p.status, p.hourly_rate, p.deadline, p.scheduled_start,
       p.invoice_cc_email, p.invoice_cc_description, p.schedule_comments,
       p.additional_info, p.additional_info2, p.discount_percent, p.discount_reason,
       p.adjustment_amount, p.adjustment_reason, p.currency_display, 
...
type SearchProjectsWithClientPaginationParams struct {
	Name   string `json:"name"`
	Name_2 string `json:"name_2"`
	Limit  int64  `json:"limit"`
	Offset int64  `json:"offset"`
...
type SearchProjectsWithClientPaginationRow struct {
	ID                     int64           `json:"id"`
	Name                   string          `json:"name"`
	ClientID               int64           `json:"client_id"`
	Status                 string          `json:"status"`
...
func (q *Queries) SearchProjectsWithClientPagination(ctx context.Context, arg SearchProjectsWithClientPaginationParams) ([]SearchProjectsWithClientPaginationRow, error) {
	rows, err := q.db.QueryContext(ctx, searchProjectsWithClientPagination,
		arg.Name,
		arg.Name_2,
		arg.Limit,
		arg.Offset,
	...
	defer rows.Close()
	items := []SearchProjectsWithClientPaginationRow{}
	for rows.Next() {
		var i SearchProjectsWithClientPaginationRow
		if err := rows.Scan(
			&i.ID,
			&i.Name,
			&i.ClientID,
